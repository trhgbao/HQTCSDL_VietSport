...
namespace VietSportSystem
{
    public class FormUpdateStatus : Form
    {
        ...
        private void InitializeComponent()
        {
            ...
            // Demo checkbox for Scenario 10 concurrency testing
            chkDemoMode = new CheckBox { Text = "Demo: Test Dirty Read (delay 10s)", Location = new Point(50, 340), AutoSize = true, ForeColor = Color.Blue, Font = new Font("Segoe UI", 9, FontStyle.Italic) };

            Button btnSave = new Button { Text = "Lưu Trạng Thái", Location = new Point(150, 380), Size = new Size(200, 50) };
            ...
            // 3. THÊM VÀO FORM
            this.Controls.AddRange(new Control[] { lblTitle, rdoGood, rdoBad, grpBad, chkDemoMode, btnSave });
        }
        ...
        private async void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            // Check if Demo mode is enabled (for Scenario 10 Testing - delay 10s before update)
            if (chkDemoMode != null && chkDemoMode.Checked)
            {
                this.Text = "Đang cập nhật... (delay 10s)";
                this.Enabled = false;

                // Delay 10s then do normal update
                await System.Threading.Tasks.Task.Run(async () =>
                {
                    await System.Threading.Tasks.Task.Delay(10000); // 10 seconds delay
                ...
                MessageBox.Show("Cập nhật thành công!", "Kết quả");
                this.Close();
                return;
            }

            // Normal flow - inline SQL (no delay)
            using (SqlConnection conn = DatabaseHelper.GetConnection())
            {
                conn.Open();
                try
                {
                    // Recalculate in case user changed options
                    if (rdoBad.Checked && string.IsNullOrEmpty(ghiChu))
                    {
                        ghiChu = $"{txtReason.Text} (Dự kiến xong: {dtpEnd.Value:dd/MM/yyyy})";
                    }
