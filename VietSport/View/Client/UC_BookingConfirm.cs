...
namespace VietSportSystem
{
    public class UC_BookingConfirm : UserControl
    {
        ...
        private CheckBox chkNonRepeatableDemo; // Demo Xung đột 3 (Non-Repeatable Read - Duy)
        // Variables
        ...
        private void InitializeComponent()
        {
            ...
            // Thêm CheckBox demo Xung đột 3 (Non-Repeatable Read) – Vị trí dưới chkFixVip
            chkNonRepeatableDemo = new CheckBox { Text = "Demo Xung đột 3 (Non-Repeatable Read)", Location = new Point(0, 280), AutoSize = true, Font = new Font("Segoe UI", 9, FontStyle.Bold), ForeColor = UIHelper.SecondaryColor };
            Label lblNote = new Label { Text = "Ghi chú:", Location = new Point(0, 310), AutoSize = true };
            txtNote = new TextBox { Location = new Point(0, 335), Width = 480, Height = 60, Multiline = true, BorderStyle = BorderStyle.FixedSingle };
            pnlLeft.Controls.AddRange(new Control[] { lblSan, lblGia, grpTime, lblDVTitle, btnService, lblServiceList, chkFixVip, chkNonRepeatableDemo, lblNote, txtNote });
            // --- CỘT PHẢI ---
        ...
        private void CalculateTotal()
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    // Chèn gọi proc Usp_TinhGiaSan nếu không demo lỗi, direct nếu demo
                    decimal giaThue = 0;
                    string maSanThuc = _sanInfo.TenSan.Split('-')[0].Trim();
                    string khungGio = "Ngày thường";  // Giả định
                    ...
                    try
                    {
                        using (SqlConnection conn = DatabaseHelper.GetConnection())
                        {
                            ...
                            using (SqlCommand cmd = new SqlCommand())
                            {
                                ...
                                {
                                    // Demo lỗi: Query direct không lock, chèn WAITFOR để pause 10s (T2 chen update)
                                    cmd.CommandText = "SELECT DonGia FROM GiaThueSan WHERE MaCoSo = (SELECT MaCoSo FROM SanTheThao WHERE MaSan = @MaSan) AND LoaiSan = (SELECT LoaiSan FROM SanTheThao WHERE MaSan = @MaSan) AND KhungGio = @KhungGio; WAITFOR DELAY '00:00:10';";
                                    cmd.Parameters.AddWithValue("@MaSan", maSanThuc);
                                    cmd.Parameters.AddWithValue("@KhungGio", khungGio);
                                    giaThue = (decimal)cmd.ExecuteScalar();
                                }
                                else
                                {
                                    // Chống lỗi: Gọi proc với lock
                                    cmd.CommandType = CommandType.StoredProcedure;
                                    cmd.CommandText = "Usp_TinhGiaSan";
                                    cmd.Parameters.AddWithValue("@MaSan", maSanThuc);
                                    cmd.Parameters.AddWithValue("@KhungGio", khungGio);
                                    SqlParameter giaParam = new SqlParameter("@GiaThue", SqlDbType.Decimal) { Direction = ParameterDirection.Output };
                                    cmd.Parameters.Add(giaParam);

                                    cmd.ExecuteNonQuery();
                                    giaThue = (decimal)giaParam.Value;
                                }
                            }
                        }
                    }
                    catch (Exception ex) { MessageBox.Show("Lỗi tính giá: " + ex.Message); giaThue = _sanInfo.GiaTien; }
